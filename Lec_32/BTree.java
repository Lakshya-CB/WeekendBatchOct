...
//		System.out.println(L + " == " + ans + " == " + R);
	...
	public void Vertical() {
		...
		Q.add(new Nodex(root));
		System.out.println("================");
		while (!Q.isEmpty()) {
			Nodex temp = Q.poll();

			System.out.println(temp.nn.data + "  " + temp.x);
//			put it in ?!
			if(mm.containsKey(temp.x)) {
				ArrayList<Integer> AL = mm.get(temp.x);
				AL.add(temp.nn.data);
			}else {
				mm.put(temp.x, new ArrayList<Integer>());
				ArrayList<Integer> AL = mm.get(temp.x);
				AL.add(temp.nn.data);	
			}
			if (temp.nn.left != null) {
				Q.add(new Nodex(temp.x - 1, temp.nn.left));
			}
			if (temp.nn.right != null) {
				Q.add(new Nodex(temp.x + 1, temp.nn.right));
			}
		}
		System.out.println("================");
